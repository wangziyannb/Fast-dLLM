...
from v2.modeling_ori import Fast_dLLM_QwenForCausalLM
...
class Fast_dLLM_v2EvalHarness(LM):
    ...
    def get_loglikelihood(self, prefix, target):
        ...
        seq = torch.cat([seq.to(self.device),
                         torch.full((seq.shape[0], self.bd_size - seq.shape[1] % self.bd_size), -100, dtype=torch.long,
                                    device=self.device)], dim=1)
