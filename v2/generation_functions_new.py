    ...
    def batch_sample(
        ...
        if min_len > block_size:
            ...
            if use_dp_prefill:
                ...
                # 3) 计算 plan（注意：block_sizes 是“整批共享”的mask规划）
                #    如果你 batch>1 且想每个 sample 不同 plan，这里需要更深改动（见下面“批处理注意事项”）。
                prefix_ids_1d = input_ids[0, :prefill_len]  # 用第0条样本做规划（最小改动）
                ...
                block_sizes_tensor = torch.tensor(plan, device=self.device, dtype=torch.long)

                # 4) prefill forward：传 block_sizes
                output = self.forward(
                    input_ids=input_ids[:, :prefill_len],
                    use_cache=True,
                    update_past_key_values=True,
                    block_size=block_size,
                    block_sizes=block_sizes_tensor,
                )
            else:
